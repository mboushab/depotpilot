...
export function AddCarDialog({ occupants }: { occupants: Option[] }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          <div role="dialog" aria-modal="true" className="w-full max-w-2xl rounded-lg border bg-card shadow-panel" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between border-b px-5 py-4">
              <h2 className="text-base font-semibold">Ajouter une voiture</h2>
            ...
            <div className="px-5 py-4">
              <ParkingForm occupants={occupants} onSuccess={() => setOpen(false)} />
            </div>
          </div>
        </div>
      ) : null}
